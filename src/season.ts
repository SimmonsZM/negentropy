...
/** Reference prices for wealth aggregation (eu per unit). The market may
 * disagree; the season's yardstick does not move mid-season. */
...
export interface SeasonComponents {
  ...
  conduct_milli: number; // 1000 = spotless (placeholder until conflict exists)
}
...
const W = { wealth: 500, feats: 200, calibration: 150, conduct: 150 } as const; // sums to 1000
...
export function computeSeason(rows: SeasonComponents[]): SeasonRow[] {
  const top = {
    wealth: Math.max(1, ...rows.map((r) => r.wealth_eu)),
